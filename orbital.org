...
#+BEGIN_SRC octave :results silent
  ...
  hold on;
  axis "off"
  print "-S720,160" "eccentricity.svg"
  hold off;
#+END_SRC
...
Once saved I still needed to fudge them to make them properly space delimined. I've saved a file with 1M years and a 100 year step size [[./year-obliq-prec-eccen-fixed.txt]]

#+BEGIN_SRC octave :results silent
  data = dlmread("year-obliq-prec-eccen-fixed.txt" , " ");
  plot(data(:,1),data(:,2))
  hold on;
  plot(data(:,1),data(:,3))
  plot(data(:,1),data(:,4).-0.3)
  axis "off"
  print "-S720,160" "laskar2004.svg"
  hold off;
#+END_SRC
