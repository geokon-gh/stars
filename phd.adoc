...
Once we have clear metric of the  _^18^O_ and monsoons we can then start quantitatively tackle the core questions:
...
- Given ideal error-free _^18^O_ value, can we decompose it into summer/winter/background fractions?
...
In our case we'd guess that the summer monsoon is "very light", the local background precipitation is "very heavy" and the winter monsoon is somewhere in between. And value of _^18^O_ can be represented by different fractions of those three climate phenomena. This is effectively an issue of too many degrees of freedom!

To constrain the problem and find a fixed solution, I need additional constraints. The constraint I theorize is that different speleothems in the same cave should exhibit different responses to precipitation
